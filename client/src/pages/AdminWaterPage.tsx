...
import { ArrowRight, BadgeCheck, ClipboardPlus, Droplets, LoaderCircle, ShieldCheck } from "lucide-react";
        ...
        {isAdmin && <>
          <section className="mb-8 rounded-3xl border border-line bg-white p-6 md:p-8">
            ...
            <form onSubmit={submitDraft} className="grid gap-4 md:grid-cols-2">
              ...
              <button type="submit" aria-busy={createDraft.isPending} disabled={createDraft.isPending} className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-falaj px-4 text-xs font-bold text-white transition-colors hover:bg-falaj-deep disabled:opacity-70 md:col-span-2">{createDraft.isPending && <LoaderCircle size={15} className="animate-spin" />} {createDraft.isPending ? "يجري حفظ المسودة" : "حفظ كمسودة للمراجعة"}</button>
            </form>
          </section>
